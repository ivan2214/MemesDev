...
export function MemeShare({
  ...
  // Construir la URL del meme
  const memeUrl = `${env.NEXT_PUBLIC_APP_URL}/meme/${memeId}`;

  // Título para compartir
  const shareTitle =
    memeTitle || `Meme divertido compartido por ${userName} 😂`;
  const shareText = `Mira este meme: ${shareTitle}`;
  ...
  const shareUrls = {
    ...
    facebook: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(memeUrl)}`,
    linkedin: `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(memeUrl)}`,
    whatsapp: `https://wa.me/?text=${encodeURIComponent(`${shareText} ${memeUrl}`)}`,
